...
/* =========================================================
   COGNISENSE CV — NO OUTER CONTAINER + FULL UI DESIGN
   Paste at VERY END of <style>
   Scope: ONLY .cs-cv
   ========================================================= */

/* -----------------------------
   1) Tokens live on .cs-cv (fine)
   BUT .cs-cv itself becomes "neutral"
   ----------------------------- */
.cs-cv{
  /* theme tokens (light) */
  --cv-text:#0b1220;
  --cv-muted:rgba(15,23,42,.68);
  --cv-muted2:rgba(15,23,42,.50);

  --cv-surface:rgba(255,255,255,.72);
  --cv-surface-strong:rgba(255,255,255,.86);
  --cv-surface-soft:rgba(255,255,255,.55);

  --cv-stroke:rgba(148,163,184,.55);
  --cv-stroke-soft:rgba(148,163,184,.30);

  --cv-cyan:#38bdf8;
  --cv-indigo:#4f46e5;
  --cv-gold:#facc15;

  --cv-shadow: 0 24px 70px rgba(15,23,42,.12);
  --cv-shadow-2: 0 18px 46px rgba(15,23,42,.10);
  --cv-shadow-3: 0 12px 26px rgba(15,23,42,.10);

  --cv-r-xl:26px;
  --cv-r-lg:20px;
  --cv-r-md:16px;

  /* REMOVE outer container feel */
...
/* kill any old overlay from previous skin */
.cs-cv::before,
.cs-cv::after{ content: none !important; }

/* Dark mode tokens */
body.dark-mode .cs-cv{
  --cv-text:#e5e7eb;
  --cv-muted:rgba(229,231,235,.72);
  --cv-muted2:rgba(229,231,235,.55);

  --cv-surface:rgba(15,23,42,.60);
  --cv-surface-strong:rgba(2,6,23,.72);
  --cv-surface-soft:rgba(2,6,23,.46);

  --cv-stroke:rgba(129,140,248,.35);
  --cv-stroke-soft:rgba(129,140,248,.18);

  --cv-shadow: 0 34px 90px rgba(0,0,0,.55);
  --cv-shadow-2: 0 24px 70px rgba(0,0,0,.45);
  --cv-shadow-3: 0 14px 38px rgba(0,0,0,.40);
}
...
/* -----------------------------
   2) INNER WRAP becomes the “panel”
   (keeps aligned width, without outer container)
   ----------------------------- */
.cs-cv .cs-wrap{
  max-width: 1320px;
  margin: 0 auto;
...
/* Subtle panel aura behind content only (not full page) */
.cs-cv .cs-wrap::before{
  content:"";
  position:absolute;
  inset: 6px;
  border-radius: calc(var(--cv-r-xl) + 8px);
  pointer-events:none;
  z-index:0;
  opacity:.85;
  background:
    radial-gradient(900px 420px at 8% 0%, rgba(56,189,248,.12), transparent 60%),
    radial-gradient(900px 420px at 92% 6%, rgba(79,70,229,.10), transparent 62%),
    radial-gradient(700px 380px at 50% 110%, rgba(250,204,21,.08), transparent 60%);
  filter: blur(10px);
}
body.dark-mode .cs-cv .cs-wrap::before{ opacity:.65; }

.cs-cv .cs-wrap > *{ position: relative; z-index: 1; }
...
.cs-cv .cs-hero{
  ...
  background: linear-gradient(135deg, rgba(11,18,38,.92), rgba(16,25,58,.90));
...
/* hero glow sweep */
.cs-cv .cs-hero::before{
  content:"";
  position:absolute;
  inset:-2px;
  pointer-events:none;
  background:
    radial-gradient(900px 420px at 18% 18%, rgba(56,189,248,.26), transparent 60%),
    radial-gradient(900px 520px at 86% 22%, rgba(167,139,250,.22), transparent 62%),
    radial-gradient(700px 420px at 50% 120%, rgba(250,204,21,.12), transparent 55%);
  filter: blur(10px);
  opacity: .9;
  animation: csHeroFloat 10s ease-in-out infinite;
}
...
@media (prefers-reduced-motion: reduce){
  .cs-cv .cs-hero::before{ animation:none !important; }
}
...
.cs-cv .cs-title{
  font-family: 'Orbitron','Poppins',sans-serif;
  font-size: clamp(1.55rem, 1.7vw + 1rem, 2.25rem);
  letter-spacing: .14em;
  text-transform: uppercase;
  margin: 0;
  line-height: 1.15;
...
.cs-cv .cs-chip b{ font-weight: 900; }

.cs-cv .cs-heroActions{
  display:flex;
  gap: 10px;
  flex-wrap: wrap;
  align-items:center;
...
.cs-cv .cs-btn{
  ...
  letter-spacing: .02em;
  overflow: hidden;
  transition: transform .16s ease, filter .18s ease, box-shadow .2s ease, border-color .2s ease;
...
.cs-cv .cs-btn:hover::after{ transform: translateX(140%); }
.cs-cv .cs-btn:active{ transform: translateY(0) scale(.99); }
...
/* Secondary buttons used inside cards */
.cs-cv .cs-btn2{
  position: relative;
  display:inline-flex;
  align-items:center;
  ...
  padding: 11px 14px;
  border-radius: 16px;
  border: 1px solid var(--cv-stroke-soft);
  background: var(--cv-surface-strong);
  color: var(--cv-text);
  font-weight: 950;
  box-shadow: var(--cv-shadow-3);
  transition: transform .16s ease, box-shadow .2s ease, border-color .2s ease, background .2s ease, filter .2s ease;
}
body.dark-mode .cs-cv .cs-btn2{
...
.cs-cv .cs-btn2:active{ transform: translateY(0) scale(.99); }

.cs-cv .cs-btn2--primary{
  background: linear-gradient(135deg, rgba(79,70,229,.98), rgba(6,182,212,.94));
  border: 0;
  color: #fff;
  box-shadow: 0 18px 44px rgba(37,99,235,.22);
}
.cs-cv .cs-btn2--primary:hover{ filter: brightness(1.03); }
...
body.dark-mode .cs-cv .cs-alert{
  background: rgba(2,6,23,.62);
  box-shadow: var(--cv-shadow-2);
}
.cs-cv .cs-alert::before{
  content:"";
  position:absolute;
  left:0; top:0; bottom:0;
  width: 6px;
  background: linear-gradient(180deg, var(--cv-cyan), var(--cv-indigo));
  opacity: .95;
}
.cs-cv .cs-alert--error::before{
  background: linear-gradient(180deg, rgba(239,68,68,.98), rgba(250,204,21,.75));
}
.cs-cv .cs-alertIcon{
...
/* =========================================================
   TYPO HELPERS
   ========================================================= */
.cs-cv .cs-muted{ color: var(--cv-muted); }
.cs-cv .cs-muted2{ color: var(--cv-muted2); }
...
.cs-cv .cs-colTips{ grid-column: span 5; }
.cs-cv .cs-colMy{ grid-column: span 7; }
...
.cs-cv .cs-card::before{
  ...
  opacity:.75;
  background:
    radial-gradient(520px 220px at 12% 0%, rgba(56,189,248,.12), transparent 60%),
    radial-gradient(520px 220px at 92% 14%, rgba(79,70,229,.10), transparent 60%);
}
.cs-cv .cs-card > *{ position: relative; z-index: 1; }
...
.cs-cv .cs-list li::before{
  content:"";
  width: 10px;
  height: 10px;
  margin-top: 7px;
  border-radius: 999px;
  background: radial-gradient(circle at 30% 30%, var(--cv-gold), var(--cv-cyan) 55%, var(--cv-indigo));
...
body.dark-mode .cs-cv .cs-cvRow{
  background: rgba(2,6,23,.42);
}
.cs-cv .cs-cvRow::before{
  content:"";
  position:absolute;
  inset:0;
  background:
    radial-gradient(520px 220px at 10% 0%, rgba(56,189,248,.10), transparent 60%),
    radial-gradient(520px 220px at 90% 15%, rgba(79,70,229,.08), transparent 60%);
  opacity:.9;
  pointer-events:none;
}
.cs-cv .cs-cvRow > *{ position: relative; z-index: 1; }

.cs-cv .cs-cvRow:hover{
  transform: translateY(-2px);
...
/* =========================================================
   TEMPLATES HEADER / TOOLBAR / SEARCH
   (you asked why it wasn't designed — here it is)
   ========================================================= */
.cs-cv .cs-toolbar{
  margin-top: 14px;
  display:flex;
...
body.dark-mode .cs-cv .cs-search{
  background: rgba(2,6,23,.55);
}
.cs-cv .cs-search::before{
  content:"";
  position:absolute;
  inset:0;
  pointer-events:none;
  background: linear-gradient(90deg, rgba(56,189,248,.10), transparent 35%, rgba(79,70,229,.08));
  opacity:.9;
}

.cs-cv .cs-search > *{ position: relative; z-index: 1; }

.cs-cv .cs-search input{
  flex: 1 1 auto;
  border: 0;
  outline: 0;
...
.cs-cv .cs-search input::placeholder{
  color: var(--cv-muted2);
  font-weight: 700;
}
...
/* =========================================================
   TEMPLATES GRID + TEMPLATE CARDS (Premium)
   ========================================================= */
.cs-cv .cs-templatesGrid{
  margin-top: 16px;
  display:grid;
  grid-template-columns: repeat(12, minmax(0, 1fr));
  gap: 16px;
...
.cs-cv .cs-templateCard:hover{
  transform: translateY(-3px);
  border-color: rgba(56,189,248,.35);
  box-shadow: 0 34px 96px rgba(2,6,23,.16);
...
.cs-cv .cs-preview{
  height: 190px;
  background:
    radial-gradient(900px 320px at 20% 10%, rgba(56,189,248,.12), transparent 55%),
    radial-gradient(700px 260px at 85% 10%, rgba(79,70,229,.10), transparent 55%),
    linear-gradient(180deg, rgba(2,6,23,.06), rgba(2,6,23,.02));
  border-bottom: 1px solid var(--cv-stroke-soft);
  display:flex;
  align-items:center;
  justify-content:center;
  padding: 12px;
...
body.dark-mode .cs-cv .cs-preview{
  background:
    radial-gradient(900px 320px at 20% 10%, rgba(56,189,248,.14), transparent 55%),
    radial-gradient(700px 260px at 85% 10%, rgba(79,70,229,.12), transparent 55%),
    linear-gradient(180deg, rgba(255,255,255,.05), rgba(255,255,255,.02));
}

.cs-cv .cs-preview img{
  width: 100%;
  height: 100%;
  object-fit: cover;
  border-radius: 18px;
...
body.dark-mode .cs-cv .cs-softCard{
  background: rgba(2,6,23,.44);
}
.cs-cv .cs-softCard::before{
  content:"";
  position:absolute;
  inset:0;
  pointer-events:none;
...
.cs-cv .cs-softCard > *{ position: relative; z-index: 1; }

.cs-cv .cs-softCard:hover{
  transform: translateY(-2px);
  border-color: rgba(56,189,248,.30);
  box-shadow: var(--cv-shadow-2);
}

.cs-cv .cs-checkItem{ display:flex; flex-direction: column; gap: 6px; }
...
/* =========================================================
   ...
   ========================================================= */
@media (max-width: 1100px){
  .cs-cv .cs-colTips{ grid-column: span 12; }
  .cs-cv .cs-colMy{ grid-column: span 12; }

  .cs-cv .cs-templateCard{ grid-column: span 6; }
  .cs-cv .cs-softCard{ grid-column: span 6; }
}

@media (max-width: 640px){
  .cs-cv .cs-heroActions{ width: 100%; justify-content: flex-start; }

  .cs-cv .cs-btn,
  .cs-cv .cs-btn2{ width: 100%; justify-content: center; }

  .cs-cv .cs-templateCard{ grid-column: span 12; }
  .cs-cv .cs-softCard{ grid-column: span 12; }

  .cs-cv .cs-preview{ height: 170px; }
  .cs-cv .cs-cvMeta{ min-width: 0; }
}

/* Light-mode: make the HERO content column purple */
body:not(.dark-mode) .cs-cv .cs-heroTop > div:not(.cs-heroActions){
  background: linear-gradient(135deg, #6d28d9 0%, #4f46e5 45%, #7c3aed 100%);
  border: 1px solid rgba(255,255,255,.20);
  border-radius: 22px;
  padding: 18px 18px;
  box-shadow: 0 22px 60px rgba(76,29,149,.28);
}

/* Keep text readable on purple */
body:not(.dark-mode) .cs-cv .cs-heroTop > div:not(.cs-heroActions) .cs-subtitle{
  color: rgba(255,255,255,.92);
}

/* Chips look a bit nicer on purple in light mode */
body:not(.dark-mode) .cs-cv .cs-heroTop > div:not(.cs-heroActions) .cs-chip{
  background: rgba(255,255,255,.14);
  border-color: rgba(255,255,255,.24);
}


/* ===== LIGHT MODE: PURPLE HERO CONTENT COLUMN ===== */
body:not(.dark-mode) .cs-heroTop > div:not(.cs-heroActions){
  background: linear-gradient(135deg, #6d28d9 0%, #4f46e5 45%, #7c3aed 100%);
  border: 1px solid rgba(255,255,255,.20);
  border-radius: 22px;
  padding: 18px;
...
body:not(.dark-mode) .cs-heroTop > div:not(.cs-heroActions) .cs-subtitle{
  color: rgba(255,255,255,.92);
}
body:not(.dark-mode) .cs-heroTop > div:not(.cs-heroActions) .cs-chip{
  background: rgba(255,255,255,.14);
  border-color: rgba(255,255,255,.24);
}

/* ===== DARK MODE: ADJUSTED PURPLE (RICHER + CLEANER CONTRAST) ===== */
.dark-mode .cs-heroTop > div:not(.cs-heroActions){
  background: linear-gradient(135deg, #2e1065 0%, #1e1b4b 55%, #4c1d95 100%);
  border: 1px solid rgba(255,255,255,.10);
  border-radius: 22px;
  padding: 18px;
  box-shadow: 0 28px 80px rgba(0,0,0,.55);
}
.dark-mode .cs-heroTop > div:not(.cs-heroActions) .cs-title{
  color: rgba(255,255,255,.96);
}
.dark-mode .cs-heroTop > div:not(.cs-heroActions) .cs-subtitle{
  color: rgba(255,255,255,.78);
}
.dark-mode .cs-heroTop > div:not(.cs-heroActions) .cs-chip{
  background: rgba(255,255,255,.08);
...
/* =========================================================
   HERO WIDTH — matches the rest of your “responsive width change”
   (wide up to sidebar; full width when sidebar collapses)
   ========================================================= */

/* Scope vars to THIS page only */
.cs-cv{
  --cs-sidebar-expanded: 280px;   /* set to your real sidebar width */
  --cs-sidebar-collapsed: 88px;  /* set to your real collapsed width */
  --cs-sidebar-current: var(--cs-sidebar-expanded);

  --cs-hero-gutter: clamp(14px, 3vw, 34px);
}

/* Toggle this class when sidebar collapses */
body.sidebar-collapsed .cs-cv{
  --cs-sidebar-current: var(--cs-sidebar-collapsed);
}

/* Mobile: sidebar overlays / disappears */
@media (max-width: 992px){
  .cs-cv{ --cs-sidebar-current: 0px; }
}

/* Avoid tiny overflow from vw math */
.cs-cv{ overflow-x: clip; }

/* Make the HERO container itself go wide up to the sidebar */
.cs-cv .cs-hero{
  max-width: none;
  width: calc(100dvw - var(--cs-sidebar-current));
  margin-left: calc(50% - 50dvw + var(--cs-sidebar-current));
  box-sizing: border-box;

  /* keep your top/bottom feel, but give it proper side gutters */
  padding: clamp(16px, 2.2vw, 26px) var(--cs-hero-gutter);
}

/* Keep heroTop as layout only (no width hacks here) */
.cs-cv .cs-heroTop{
  display:flex;
  align-items:flex-start;
  justify-content: space-between;
...
/* LEFT HERO CONTENT column (purple default in light mode) */
body:not(.dark-mode) .cs-cv .cs-heroTop > div:not(.cs-heroActions){
  background: linear-gradient(135deg, #6d28d9 0%, #4f46e5 45%, #7c3aed 100%);
  border: 1px solid rgba(255,255,255,.20);
  border-radius: 22px;
...
/* Dark mode: richer purple, cleaner contrast */
body.dark-mode .cs-cv .cs-heroTop > div:not(.cs-heroActions){
  background: linear-gradient(135deg, #2e1065 0%, #1e1b4b 55%, #4c1d95 100%);
  border: 1px solid rgba(255,255,255,.10);
  border-radius: 22px;
  padding: 18px;
...
/* === FIX: stretch the PURPLE hero content to the same right edge === */
.cs-cv .cs-heroTop{
  width: 100%;
}

/* This is the left content <div> (title/subtitle/chips) */
.cs-cv .cs-heroTop > div:not(.cs-heroActions){
  flex: 1 1 0 !important;   /* allow it to grow */
  width: 100% !important;  /* make it span the row */
  min-width: 0;
  align-self: stretch;
}
...
<div class="profile-actions" style="justify-content: flex-end;">
    ...
    {{-- HERO --}}
      ...
      <div class="cs-heroTop">
        <div>
          <h1 class="cs-title">Generate CV — Cognisense</h1>
          <p class="cs-subtitle">
            Choose a premium template, fill your credentials once, and get a polished, ATS-friendly PDF instantly.
            We also provide smart tips so your CV looks professional and reads like a winner.
          </p>

          <div class="cs-chipRow" aria-label="Quick stats">
            <span class="cs-chip">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                <path d="M4 6h16M4 12h10M4 18h16" stroke="rgba(255,255,255,.9)" stroke-width="2" stroke-linecap="round"/>
              </svg>
              <span><b>{{ count($templates) }}</b> templates</span>
            </span>
            <span class="cs-chip">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                <path d="M7 7h10v14H7z" stroke="rgba(255,255,255,.9)" stroke-width="2" />
                <path d="M9 3h6v4H9z" stroke="rgba(255,255,255,.9)" stroke-width="2" />
              </svg>
              <span><b>{{ count($myCvs) }}</b> generated</span>
            </span>
            <span class="cs-chip">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                <path d="M12 2l3 7h7l-5.5 4.1L18.5 21 12 16.9 5.5 21l2-7.9L2 9h7z" stroke="rgba(255,255,255,.9)" stroke-width="2" stroke-linejoin="round"/>
              </svg>
              <span><b>ATS</b> friendly</span>
            </span>
          </div>
        </div>
    ...
    {{-- Alerts --}}
    @if(session('success'))
      <div class="cs-alert cs-alert--success" role="status" aria-live="polite">
        <div class="cs-alertIcon" aria-hidden="true">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
            <path d="M20 6L9 17l-5-5" stroke="rgba(34,197,94,.95)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
        </div>
        <div>
          <div style="font-weight:950;">Success</div>
          <div class="cs-muted" style="margin-top:2px;">{{ session('success') }}</div>
        </div>
      </div>
    @endif
    ...
    @if(session('error'))
      <div class="cs-alert cs-alert--error" role="alert">
        <div class="cs-alertIcon" aria-hidden="true">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
            <path d="M12 9v4m0 4h.01M10.3 3.5h3.4L22 19.2A2 2 0 0 1 20.3 22H3.7A2 2 0 0 1 2 19.2L10.3 3.5z"
                  stroke="rgba(239,68,68,.95)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
        </div>
        <div>
          <div style="font-weight:950;">Error</div>
          <div class="cs-muted" style="margin-top:2px;">{{ session('error') }}</div>
        </div>
      </div>
    @endif
    ...
    {{-- Tips + My CVs --}}
    <div class="cs-grid">
      <div class="cs-card cs-colTips">
        <div class="cs-sectionHead">
          <div>
            <div class="cs-h3">CV Benefits & Pro Tips</div>
            <div class="cs-muted" style="margin-top:6px; line-height:1.55;">
              Build a CV that’s clean, credible, and easy for both ATS and recruiters to scan.
            </div>
          </div>
          <span class="cs-pill">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              <path d="M12 2l3 7h7l-5.5 4.1L18.5 21 12 16.9 5.5 21l2-7.9L2 9h7z" stroke="rgba(15,23,42,.85)" stroke-width="2" stroke-linejoin="round"/>
            </svg>
        ...
        <ul class="cs-list">
          <li><b>Lead with impact:</b> start bullets with action verbs and include numbers (%, time, money, users).</li>
          <li><b>ATS keywords:</b> match skills/tools from the job description (don’t fake—be honest).</li>
          <li><b>1–2 pages max:</b> students/entry: 1 page. Experienced: 1–2 pages.</li>
          <li><b>Clean structure:</b> strong headings, consistent dates, and readable spacing.</li>
          <li><b>Projects matter:</b> for software roles, showcase 2–4 projects with stack + results.</li>
          <li><b>Proof:</b> add portfolio/GitHub/LinkedIn; recruiters love clickable credibility.</li>
        </ul>

        <div class="cs-actions">
          <a class="cs-btn2" href="#mycvs">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              <path d="M12 5v14M19 12l-7 7-7-7" stroke="rgba(15,23,42,.85)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
            View My Generated CVs
          </a>
          <a class="cs-btn2 cs-btn2--primary" href="#templates">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              <path d="M4 6h16M4 12h16M4 18h10" stroke="rgba(255,255,255,.95)" stroke-width="2" stroke-linecap="round"/>
            </svg>
            Pick a Template
          </a>
        </div>
      ...
      <div class="cs-card cs-colMy" id="mycvs">
        <div class="cs-sectionHead">
          <div>
            <div class="cs-h3">Your Generated CVs</div>
            <div class="cs-muted" style="margin-top:6px;">Open online or download instantly.</div>
          </div>
          <span class="cs-pill cs-pill--mono">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              <path d="M7 7h10v14H7z" stroke="rgba(15,23,42,.85)" stroke-width="2" />
              <path d="M9 3h6v4H9z" stroke="rgba(15,23,42,.85)" stroke-width="2" />
            </svg>
            Saved
          </span>
        </div>
        ...
        @if(count($myCvs) === 0)
          <div class="cs-cvRow" style="margin-top:14px;">
            <div class="cs-cvMeta">
              <div class="cs-cvTitle">No CVs generated yet</div>
              <div class="cs-cvSmall">Pick a template below and generate your first CV.</div>
            </div>
            <div class="cs-actions" style="margin-top:0;">
              <a class="cs-btn2 cs-btn2--primary" href="#templates">
                Browse Templates
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                  <path d="M9 18l6-6-6-6" stroke="rgba(255,255,255,.95)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
              </a>
            </div>
          </div>
        @else
          @foreach($myCvs as $cv)
            <div class="cs-cvRow">
              <div class="cs-cvMeta">
                <div class="cs-cvTitle">{{ $cv->title ?? 'Cognisense CV' }}</div>

                <div class="cs-inlinePills">
                  <span class="cs-pill cs-pill--mono">
                    Template: <b>{{ $cv->template_slug }}</b>
                  </span>
                  @php
                    $isGood = strtolower((string)$cv->status) === 'ready' || strtolower((string)$cv->status) === 'completed' || strtolower((string)$cv->status) === 'generated';
                  @endphp
                  <span class="cs-pill {{ $isGood ? 'cs-pill--good' : 'cs-pill--mono' }}">
                    Status: <b>{{ $cv->status }}</b>
                  </span>
                  <span class="cs-pill cs-pill--mono">
                    Created: <b>{{ $cv->created_at }}</b>
                  </span>
                </div>
              </div>

              <div class="cs-actions" style="margin-top:0;">
                <a class="cs-btn2 cs-btn2--primary" href="{{ route('cv.show', ['uuid' => $cv->uuid]) }}" target="_blank" rel="noopener">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                    <path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7-10-7-10-7z" stroke="rgba(255,255,255,.95)" stroke-width="2"/>
                    <path d="M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6z" stroke="rgba(255,255,255,.95)" stroke-width="2"/>
                  </svg>
                  Open
                </a>
                <a class="cs-btn2" href="{{ route('cv.download', ['uuid' => $cv->uuid]) }}">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                    <path d="M12 3v10m0 0l4-4m-4 4l-4-4" stroke="rgba(15,23,42,.85)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M5 21h14" stroke="rgba(15,23,42,.65)" stroke-width="2" stroke-linecap="round"/>
                  </svg>
                  Download
                </a>
              </div>
    ...
    {{-- Templates Header --}}
    <div class="cs-card" style="margin-top:16px;" id="templates">
      <div class="cs-sectionHead">
        <div>
          <h2 class="cs-h2">Choose a Template</h2>
          <div class="cs-muted" style="margin-top:6px;">
            Add more templates by inserting into <b>cv_templates</b>.
          </div>
        </div>
        <span class="cs-pill cs-pill--mono" id="csTemplateCount">
          Showing: <b>{{ count($templates) }}</b>
        </span>
      </div>

      <div class="cs-toolbar">
        <div class="cs-search" role="search" aria-label="Search templates">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
            <path d="M21 21l-4.3-4.3m1.3-5.2a7 7 0 1 1-14 0 7 7 0 0 1 14 0z"
                  stroke="rgba(15,23,42,.65)" stroke-width="2" stroke-linecap="round"/>
          </svg>
          <input id="csTemplateSearch" type="text" placeholder="Search by name, slug, description…" autocomplete="off" />
          <span class="cs-kbd">Ctrl / ⌘ + K</span>
        </div>
    ...
    {{-- Templates Grid --}}
    <div class="cs-templatesGrid" id="csTemplatesGrid" aria-label="Templates">
      @foreach($templates as $t)
        <div class="cs-template cs-templateCard"
             data-name="{{ strtolower($t->name) }}"
             data-slug="{{ strtolower($t->slug) }}"
             data-desc="{{ strtolower($t->description ?? '') }}">
          <div class="cs-preview" aria-label="Template preview">
            @if(!empty($t->preview_image))
              <img src="{{ asset($t->preview_image) }}" alt="{{ $t->name }}">
            @else
              <span style="padding:10px;text-align:center">{{ strtoupper($t->name) }}</span>
            @endif
          </div>

          <div class="cs-templateTop">
            <div style="min-width:0;">
              <div class="cs-templateName">{{ $t->name }}</div>
              <div class="cs-templateDesc">{{ $t->description }}</div>
            </div>
            <span class="cs-pill cs-pill--mono">{{ $t->slug }}</span>
          </div>

          <div class="cs-templateActions">
            <a class="cs-btn2 cs-btn2--primary" href="{{ route('cv.create', ['slug' => $t->slug]) }}">
              Use Template
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                <path d="M9 18l6-6-6-6" stroke="rgba(255,255,255,.95)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
    ...
    <div class="cs-card" style="margin-top:16px;">
      <div class="cs-sectionHead">
        <div>
          <div class="cs-h3">Recruiter Checklist (Quick)</div>
          <div class="cs-muted" style="margin-top:6px;">A fast sanity-check before you generate and send.</div>
        </div>
        <span class="cs-pill">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden="true">
            <path d="M20 6L9 17l-5-5" stroke="rgba(15,23,42,.85)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
          Ready to apply
        </span>
      </div>

      <div class="cs-checkGrid">
        <div class="cs-softCard cs-checkItem">
          <div class="cs-checkIcon" aria-hidden="true">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
              <path d="M4 6h16M4 12h10M4 18h16" stroke="rgba(29,78,216,.9)" stroke-width="2" stroke-linecap="round"/>
            </svg>
          </div>
          <b style="font-weight:950;">Format</b>
          <div class="cs-muted" style="margin-top:6px; line-height:1.55;">
            Consistent dates, clean headings, readable font, no clutter.
          </div>
        </div>

        <div class="cs-softCard cs-checkItem">
          <div class="cs-checkIcon" aria-hidden="true" style="background: rgba(34,197,94,.10); border-color: rgba(34,197,94,.14); color:#166534;">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
              <path d="M12 2l3 7h7l-5.5 4.1L18.5 21 12 16.9 5.5 21l2-7.9L2 9h7z"
                    stroke="rgba(22,101,52,.9)" stroke-width="2" stroke-linejoin="round"/>
            </svg>
          </div>
          <b style="font-weight:950;">Impact</b>
          <div class="cs-muted" style="margin-top:6px; line-height:1.55;">
            Use results + numbers: “Reduced API latency by 35%”.
          </div>
        </div>

        <div class="cs-softCard cs-checkItem">
          <div class="cs-checkIcon" aria-hidden="true" style="background: rgba(124,58,237,.10); border-color: rgba(124,58,237,.14); color:#5b21b6;">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
              <path d="M12 3l8 4v6c0 5-3.5 8-8 8s-8-3-8-8V7l8-4z"
                    stroke="rgba(91,33,182,.9)" stroke-width="2" stroke-linejoin="round"/>
              <path d="M9.5 12l1.8 1.8L15.5 9.6" stroke="rgba(91,33,182,.9)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
          </div>
          <b style="font-weight:950;">Relevance</b>
          <div class="cs-muted" style="margin-top:6px; line-height:1.55;">
            Tailor keywords to the job; keep only relevant content.
          </div>
        </div>
      </div>
    </div>
...
<script>
/* Lightweight UX only (does not change backend logic):
   - Ctrl/⌘+K focuses search
   - Live template filtering
   - Smooth scroll for in-page anchors
*/
(function(){
  const search = document.getElementById('csTemplateSearch');
  const grid = document.getElementById('csTemplatesGrid');
  const countEl = document.getElementById('csTemplateCount');
  if (!search || !grid || !countEl) return;

  const cards = Array.from(grid.querySelectorAll('.cs-templateCard'));

  function updateCount(visible){
    countEl.innerHTML = 'Showing: <b>' + visible + '</b>';
  }
